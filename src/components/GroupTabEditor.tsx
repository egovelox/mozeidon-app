...
export const GroupTabEditor = ({
  ...
  const handleClickExistingGroup = (groupId: number) => async () => {
    ...
    const newIndex = isTabMovingDown
      ? firstTabOfGroup.index - 1
      : lastTabOfGroup.index + 1
    ...
    for (const groupItemsCopyGroup of groupItemsCopy) {
      for (const groupedTab of groupItemsCopyGroup.tabs) {
        if (groupedTab.type === "tab" && groupedTab.id !== tab.id) {
          groupedTab.index =
            groupedTab.index + getIndexShift(oldIndex, newIndex, groupedTab)
        }
      }
    }
    if (!group.collapsed) {
      const previousTabIndex = baseItems.findIndex(
        isTabMovingDown
          ? (t) => t.id === firstTabOfGroup.id
          : (t) => t.id === lastTabOfGroup.id
      )
      if (isTabMovingDown) {
    ...
    for (const listCopyTab of listCopy) {
      const groupIndex = groupItemsCopy.findIndex(
        (g) => g.id === listCopyTab.groupId
      )
      if (groupIndex !== -1) {
      ...
      listCopyTab.index =
        listCopyTab.index + getIndexShift(oldIndex, newIndex, listCopyTab)
    }
    ...
    <div className="groupTabEditorView">
      <div className="settingsContent settingsContentSmall">
        {groupItems.length > 0 && (
          ...
          >
            <h4>
              <span className="mozeidonColor">{`${tab.title} `}</span> <br />{" "}
              <br />
              Move this tab inside one of your groups ...
            </h4>
            <div
              style={{ display: "flex", flexWrap: "wrap", fontSize: ".9em" }}
            >
              {groupItems.map((g, i) => {
                return (
                  <button
        ...
        <div
          style={{ display: "flex", flexDirection: "column", fontSize: ".9em" }}
        >
          <br />
